  ...
  return (
    <AppShell action={<RunMonitorButton />} sourceLabel={portfolio?.source === "fortyguard" ? "FortyGuard data" : "Synthetic demo data"}>
      <div className="app-page portfolio-page">
        ...
        <section className="portfolio-note"><span>Demo boundary</span><p>The judge-demo control stays synthetic. The separate FortyGuard action requests verified hourly field means and keeps every resulting payout labelled simulated.</p><Link href="/app/evidence/demo-042">Open example evidence <ArrowUpRight size={15} strokeWidth={1.4} /></Link></section>
        <section className="scenario-control"><div><p className="app-eyebrow">Judge demo control</p><h2>Run the five-hour heat-wave scenario.</h2><p>Loads the fixed North Field readings, applies the policy, and opens the evidence handoff.</p></div><div className="scenario-control-actions"><button onClick={startMonitoring} disabled={isRunning || liveMonitoring.isPending}><span>North Field</span>{isRunning ? "Monitoring…" : "Run scenario"}<ArrowUpRight size={16} /></button><button className="live-monitor-button" onClick={() => { setLiveStatus("Requesting three verified hourly FortyGuard temperature means…"); liveMonitoring.mutate(); }} disabled={isRunning || liveMonitoring.isPending}><span>FortyGuard</span>{liveMonitoring.isPending ? "Fetching…" : "Fetch live data"}<ArrowUpRight size={16} /></button>{liveStatus && <p className="live-monitor-status" role="status">{liveStatus}</p>}</div></section>
      </div>
    </AppShell>
  );
}
